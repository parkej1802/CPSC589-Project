...
void GPU_Geometry3D::setVerts(const std::vector<glm::vec3>& verts) {
	vertBuffer.uploadData(sizeof(glm::vec3) * verts.size(), verts.data(), GL_STATIC_DRAW);
}


void GPU_Geometry3D::setCols(const std::vector<glm::vec3>& cols) {
	colBuffer.uploadData(sizeof(glm::vec3) * cols.size(), cols.data(), GL_STATIC_DRAW);
}

void GPU_Geometry3D::setUVs(const std::vector<glm::vec2>& uvs) {
	uvBuffer.uploadData(sizeof(glm::vec2) * uvs.size(), uvs.data(), GL_STATIC_DRAW);
}

void GPU_Geometry3D::setNormals(const std::vector<glm::vec3>& norms) {
